...
public abstract class AbstractEnhancedDeserializationProxy {
    ...
    protected AbstractEnhancedDeserializationProxy(Class<?> type, Map<String, cn.lnd.ibatis.executor.loader.ResultLoaderMap.LoadPair> unloadedProperties,
                ...
                synchronized (this.reloadingPropertyLock) {
                    if (!FINALIZE_METHOD.equals(methodName) && PropertyNamer.isProperty(methodName) && !reloadingProperty) {
                        ...
                        if (unloadedProperties.containsKey(propertyKey)) {
                            final cn.lnd.ibatis.executor.loader.ResultLoaderMap.LoadPair loadPair = unloadedProperties.remove(propertyKey);
                            if (loadPair != null) {
                                try {
